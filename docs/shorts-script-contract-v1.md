...
台本生成時にLLMが0〜6件のClip候補を提案する方式をPrimaryとします。通常回では原則6件を目標にし、切れ高不足・重複・前後文脈依存が強い場合のみ減らします。
完成後の追加解析はSecondary（将来拡張）です。公開判断と候補抽出は分離します。

1. 会話本文を完成させ、Parserの1始まりのTurn番号を数える。
2. 独立して伝わるフックから、着地・発見・ツッコミ・余韻までを候補にする。
...
Shorts候補は単に「長めに綺麗にまとまっている部分」を選ぶものではありません。15〜25秒を中心に、上限30秒程度でパンチラインが立つ区間を優先します。

壁ラジでは、本編の中に自然に生まれた「思考の移動」が短時間で見える区間を優先してください。
特に次のような区間は有力な候補です。

- 普通の話題から、予想外に具体的なものへ着地する
- 抽象的な話と生活感のある単語が急につながる
- HostとGuestの認識のズレやツッコミによって話が転がる
- 一見どうでもいい発端が、妙に大きな話へ広がる
- 前後を知らなくても「なんでそうなった？」と少し気になる
- 元ログ特有の言い回しや妙な具体性が残っている

候補は必ずしも綺麗な結論で終わる必要はありません。
疑問、ツッコミ、妙な着地、少し残る余韻もShortsとして成立する終点として扱えます。

通常回では、まず異なる切り口で6候補を取れないか検討してください。数合わせのために弱い候補を作るのではなく、一発ネタ、生活あるある、思考の移動、認識のズレ、メタ発言、妙な具体語や余韻など、性格の違う候補を探して候補数を最大化します。

ただし、Shorts候補を作るために本編へ不自然なギャグ、煽り、オチ、説明を追加してはいけません。
本編として自然に成立した会話から候補を抽出してください。

似た性質の候補ばかりを選ばず、異なる魅力を持つ区間がある場合は候補を分散させてください。
...
| `displayTitle` | 画面用見出し。1〜2行、1行9文字以内。推奨6〜8文字、`\n`で改行。まず内容や違和感が一瞬で伝わることを優先し、そのうえで元会話に特徴的な具体語・認識のズレ・意外な組み合わせがある場合は積極的に活用する。具体語を使うために分かりやすさを犠牲にしない |
| `startTurn` / `endTurn` | Parser文書順、1始まり、両端を含む整数 |
| `hook` / `point` | 選定理由の編集メモ。映像に表示せず、読み上げない |
| `targetDuration` | 15〜25秒を中心とし、上限30秒程度を目安とする編集上の目標。実時間は音声生成後に決まる |
