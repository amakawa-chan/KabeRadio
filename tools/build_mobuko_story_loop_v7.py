...
DRINK_PATHS = [
    ASSET_ROOT / f"mobuko-radio-drink-v7-{index:02d}.png"
...
FPS = 24
FRAME_DIR = ROOT / "_tmp_mobuko_story_v7"
DRINK_FRAME_DIR = ROOT / "_tmp_mobuko_drink_v7"
STRETCH_FRAME_DIR = ROOT / "_tmp_mobuko_stretch_v7"
...
def add_cels(
    ...
    """Append complete held cels without interpolation or crossfades."""
...
def main() -> None:
    ...
    # Reach, lift, bring the mug to the lips, then take one quiet sip.
    add_reversible_action(frames, base, drink, transition_hold=8, pose_hold=36)
    add_cels(frames, [base], hold=120)

    add_reversible_action(frames, base, cheek, transition_hold=8, pose_hold=72)
    add_cels(frames, [base], hold=120)

    # Sit upright, lift the elbows, and finish with hands above the head.
    add_reversible_action(frames, base, stretch, transition_hold=10, pose_hold=48)
    add_cels(frames, [base], hold=120)
    ...
    add_cels(frames, [base], hold=93)
    assert len(frames) == 1200, len(frames)
    write_frames(frames, FRAME_DIR)

    drink_preview = action_preview(base, drink, transition_hold=8, pose_hold=36)
    stretch_preview = action_preview(base, stretch, transition_hold=10, pose_hold=48)
